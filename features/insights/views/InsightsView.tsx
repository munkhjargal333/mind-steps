// features/insights/views/InsightsView.tsx
'use client';
    ...
    return (
      <div className="w-full max-w-2xl mx-auto px-4 py-6">
        <div className="flex flex-col items-center justify-center py-20 gap-5 text-center">
          ...
          <Button onClick={() => router.push('/upgrade')} className="rounded-2xl px-6">
      ...
      {!loading && insights.length > 0 && (
        <ul className="space-y-4">
          {insights.map((insight) => (
            <li key={insight.id} className="p-5 rounded-2xl bg-card border border-border space-y-2">
              <p className="text-[11px] text-muted-foreground">{relativeTimeMn(insight.created_at)}</p>
              <p className="text-sm leading-relaxed text-foreground/85">{insight.content}</p>
            </li>
          ))}
        </ul>
      )}
