// features/entries/components/EntryCard.tsx
// ─────────────────────────────────────────────────────────────────────────────
// EntryCardSkeleton-г shared/ui/skeleton-аас авсан хувилбар.
// ─────────────────────────────────────────────────────────────────────────────

'use client';

import { Trash2, Lock, FileText, Loader2, Brain } from 'lucide-react';
import { Button } from '@/shared/ui/button';
import Link from 'next/link';
import { relativeTimeMn } from '@/shared/lib/date';

// Skeleton shared-аас import хийнэ
export { EntryCardSkeleton } from '@/shared/ui/skeleton';
...
interface EntryCardProps {
  entry: {
    ...
    entry_index: number;
    created_at: string;
    /** Optional: dominant emotion key (e.g. 'joy', 'fear') */
    dominant_emotion?: string | null;
    /** Optional: whether AI analysis was run */
    has_insight?: boolean;
  };
...
// Emotion emoji map (хялбар хувилбар)
const EMOTION_EMOJI: Record<string, string> = {
  joy: '☀️', trust: '🤝', fear: '🌫️', surprise: '✨',
  sadness: '🌧️', disgust: '🍂', anger: '🔥', anticipation: '🌅',
};

export function EntryCard({ entry, onDelete, deleting }: EntryCardProps) {
  const preview = entry.surface_text?.slice(0, 120) ?? null;
  const emotionEmoji = entry.dominant_emotion ? EMOTION_EMOJI[entry.dominant_emotion] : null;
  ...
  return (
    <div className="group relative flex gap-4 p-4 rounded-2xl border bg-card hover:border-primary/40 transition-all">
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Emotion + insight тэмдэглэгээ */}
        {(emotionEmoji || entry.has_insight) && (
          <div className="flex items-center gap-1.5 mt-1.5">
            {emotionEmoji && (
              <span
                className="text-[13px] leading-none"
                title={entry.dominant_emotion ?? undefined}
              >
                {emotionEmoji}
              </span>
            )}
            {entry.has_insight && (
              <span className="flex items-center gap-1 text-[10px] text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30 px-2 py-0.5 rounded-full">
                <Brain size={9} />
                Анализ хийгдсэн
              </span>
            )}
